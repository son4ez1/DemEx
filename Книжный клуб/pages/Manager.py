...
class Manager (QDialog):
    def __init__(self, table, typeUs): #[1]- типо сноски typeUs менять из WelcomeScreen
        ...
        self.tableWidget_Manager = table #tableWidget_Klient менять 
        self.vivod(typeUs)
    ...
    def vivod(self, typeUs):
        ...
        self.tableWidget_Manager.setColumnCount(len(name_stolba)) #tableWidget_Klient менять
        self.tableWidget_Manager.setHorizontalHeaderLabels(name_stolba) #tableWidget_Klient менять

        dan_table= cur.fetchall()
        
        self.tableWidget_Manager.setRowCount(0) #tableWidget_Klient менять
        # row - строки
        for i, row in enumerate(dan_table): #цикл по строкам
            self.tableWidget_Manager.setRowCount(self.tableWidget_Manager.rowCount() + 1) #tableWidget_Klient менять
            for l, cow in enumerate(row): #начинает по ячейке заносить данные
                self.tableWidget_Manager.setItem(i,l,QTableWidgetItem(str(cow))) #tableWidget_Klient менять
        print(dan_table)

        self.tableWidget_Manager.resizeColumnsToContents() #tableWidget_Klient менять
