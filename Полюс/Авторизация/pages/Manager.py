...
class Manager(QDialog):
    def __init__(self, tadle):        
        ...
        print("ok")
        self.tableWidget_Manager = tadle
        print(tadle)
        self.vivod()
    ...
    def vivod(self):
        ...
        self.tableWidget_Manager.setColumnCount(len(name_stolba)) # считает количество столбцов
        self.tableWidget_Manager.setHorizontalHeaderLabels(name_stolba) # вместо цифр подставляет название столбцов
        ...
        for i, row in enumerate(dan_table): # цикл по строчкам
            self.tableWidget_Manager.setRowCount(self.tableWidget_Manager.rowCount() + 1) # добавил пустые строки в нужном количестве
            for l, cow in enumerate(row): # по ячейкам заносит данные
                self.tableWidget_Manager.setItem(i,l, QTableWidgetItem(str(cow))) 
        print(dan_table)

        self.tableWidget_Manager.resizeColumnsToContents() # столбцы стал по размеру данных
